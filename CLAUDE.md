...
- **Focus immédiat** : éprouver la saisie à l'échelle visée, jamais tentée au-delà de quelques dizaines de cartes (`api/app/measure/typing_pace.py` reconstitue le rythme depuis `added_at`). Côté Riftbound, trois manques avant que la boucle de valeur tienne : les prix, un corpus de decks, et le gabarit d'illustration — sans lui la reconnaissance ne peut pas fonctionner, l'empreinte y étant la voie principale faute de catalogue traduit. Un chantier Magic reste ouvert : rien ne détecte les bords de la carte dans la photo, si bien que l'empreinte exige un cadrage à 2-3 % près — l'index, lui, couvre désormais les 49 484 illustrations distinctes du catalogue.
