...
- **Classeurs** : l'onglet Collection a deux vues — la liste, et le **classeur**. Un classeur est une édition, une case est le couple (extension, numéro) : rien n'est stocké, tout se dérive de `card_prints` (`my_binder_shelf`, `my_binder_page`). Ce qu'il montre et qu'une liste ne montre pas, ce sont les **cases vides** : la page part du catalogue, pas de la collection. L'entrée est une **étagère** des seules éditions où quelque chose est rangé — les 690 autres seraient vides. Le brillant ne dédouble pas la case, il s'y signale. Le volume et les pages qui se tournent restent à faire.
