...
void main() {
  ...
  group('la feuille penche du côté par lequel on la prend', () {
    // Une page prise par un coin ne se lève pas comme une page prise au
    // milieu : le coin saisi part le premier.
    test('prise en son milieu, elle ne penche pas', () {
      expect(leafTilt(t: 0.5, grabY: 0.5), 0);
    });

    test('les deux moitiés penchent en sens contraires', () {
      final haut = leafTilt(t: 0.5, grabY: 0);
      final bas = leafTilt(t: 0.5, grabY: 1);
      expect(haut, isNot(0));
      expect(bas, -haut);
    });

    test('elle est droite quand elle repose, à plat comme sur la pile', () {
      // L'inclinaison naît et meurt avec le mouvement : sinon la feuille
      // arriverait de travers sur la pile et y resterait.
      //
      // `closeTo` et non l'égalité : la fin du mouvement passe par `sin(π)`,
      // qui vaut 1,3·10⁻¹⁷ en virgule flottante — nul à l'écran comme au
      // radian près, mais pas au bit près.
      expect(leafTilt(t: 0, grabY: 1), closeTo(0, 1e-12));
      expect(leafTilt(t: 1, grabY: 1), closeTo(0, 1e-12));
    });

    test('penche le plus au sommet du mouvement', () {
      final milieu = leafTilt(t: 0.5, grabY: 1).abs();
      for (final t in [0.1, 0.25, 0.75, 0.9]) {
        expect(leafTilt(t: t, grabY: 1).abs(), lessThan(milieu));
      }
    });

    test('reste sous une inclinaison qu\'un classeur admettrait', () {
      // Au-delà, la page paraît décrochée de sa reliure plutôt que tenue.
      for (final grab in [0.0, 0.25, 0.5, 0.75, 1.0]) {
        for (final t in [0.0, 0.3, 0.5, 0.8, 1.0]) {
          expect(leafTilt(t: t, grabY: grab).abs(), lessThan(0.2));
        }
      }
    });

    test('une saisie hors de la feuille ne la fait pas basculer', () {
      // Les coordonnées viennent d'un geste, qui peut déborder du cadre.
      expect(leafTilt(t: 0.5, grabY: -3), leafTilt(t: 0.5, grabY: 0));
      expect(leafTilt(t: 0.5, grabY: 9), leafTilt(t: 0.5, grabY: 1));
    });
  });
}
