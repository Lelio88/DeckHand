...
| carte couchée | 12 | 3,6 % | 12,5 bits | 9 bits | 6 / 11 |
| carte à l'envers | 1 | 2,3 % | 7 bits | 9 bits | 1 / 1 |
| **toutes** | **32** | 9,5 % | 16 bits | **8,5 bits** | **10 / 26** |

**C'est la géométrie**, et le diagnostic a été confirmé en le corrigeant. Avant
l'ouverture des orientations (§ suivant), les cartes couchées affichaient un
écart médian de **128,3 %** pour 30 bits, et **aucune** ne passait ; les mêmes
photos, avec la seule fenêtre remise d'aplomb, tombent à 3,6 % et 12,5 bits. Rien
d'autre n'a changé : ni l'empreinte, ni ses seuils, ni la détection.

L'empreinte n'est donc pas le facteur limitant — elle est prête à reconnaître une
carte que le cadrage lui montre correctement. Ce que la colonne « vraie fenêtre »
mesure reste le plafond à viser : **26 photos sur 32** y passent sous le seuil.
...
#### L'orientation : deux sens par gabarit, choisis par le rapport

Treize photos sur trente-deux montrent un carton **couché ou à l'envers**. Le
rapport du quadrilatère décide donc des sens à essayer, et n'en laisse que deux :

| gabarit \ quadrilatère | debout | couché |
|---|---|---|
| **droit** (Magic, Yu-Gi-Oh…) | 0 et 2 | 1 et 3 |
| **couché** (champ de bataille, Terrain…) | 1 et 3 | 0 et 2 |

Il en faut deux et pas un : une empreinte ne survit pas au demi-tour. Il n'en
faut pas quatre : les deux autres sens prélèveraient l'empreinte sur une zone
qu'aucune carte n'occupe, et chaque hypothèse est un tirage de plus dans l'index.
...
| **deux sens guidés par le rapport** | **8** | **1** | **0** |
| quatre sens, à l'aveugle | 8 | 2 | 1 |

La règle guidée domine les deux autres sur les trois colonnes. La raison se lit
dans le détail : les deux cartes annoncées à tort par l'ancienne règle gagnaient
**au tour 0 sur un quadrilatère couché** — le sens géométriquement impossible.
L'ouvrir n'apporte que le faux positif ; le fermer en supprime un.

L'ancien refus de la réciproque — « une carte debout ne se présente pas couchée,
donc un quadrilatère couché signale une détection fausse » — avait donc tort sur
la prémisse et **raison sur le danger**. C'est ce qui donne à la règle sa forme :
le sens impossible est *remplacé*, jamais complété.

**Le flux caméra garde la règle fermée**, et c'est écrit dans `live_scanner.dart`
plutôt que subi : une carte Magic couchée n'y parvient jamais, l'aspect la
rejetant en amont, et un tirage de plus n'y pèse pas le même poids à trente
...
| Carte couchée ou à l'envers **dans le flux caméra** | Structurelle : l'aspect la rejette avant toute hypothèse | Le mode photo la traite (deux sens par gabarit, choisis par le rapport) ; le flux, non — il n'accepte que l'orientation que son capteur lui apprend. Ouvrir davantage à trente images par seconde demande une mesure sur l'appareil, pas un raisonnement. Voir « Le plafond d'identification ». |
...
**`card_printings` sert ces éditions par pages** (`p_limit`, `p_offset`), et le sélecteur charge la suite quand on la demande. 19 cartes Magic dépassent les 60 éditions d'une page — les cinq terrains de base près de 900 chacun, Sol Ring 135 —, si bien que relever le plafond n'aurait fait que déplacer le problème : il reste des Forêts au-delà des 200 lignes que le serveur accepte, même dans une seule tranche d'années. Mesuré, le nombre de lignes ne change pas le temps serveur (0,7 s pour la Forêt à 60 comme à 200), seulement le volume rapatrié. **La finition se filtre côté serveur** (`p_finish`) : filtrée après la coupe, une page de 60 éditions pouvait n'en montrer que dix en brillant, et la pagination ne tient que si chaque page compte ce qu'elle promet.

**L'aperçu en grand demande l'illustration d'une édition au lieu de la chercher dans ces pages** (`edition_art(print_id)`). Cherchée dans la première, une édition hors de celle-ci passait pour sans image, et le bandeau de repli l'affirmait. La fonction regarde toute la case — une fiche localisée publiée sans image garde l'illustration d'une autre langue de la même édition — et ne rend `NULL` que pour une édition réellement sans image : alors seulement l'aperçu montre celle d'une autre édition, et le dit. Les écrans d'étalement et de dictée lui passent l'édition choisie, comme la recherche.
...
**« + » enregistre l'édition que la ligne affiche, et la ligne en propose une.** Sans geste, l'écran Ajouter envoyait tout dans la pile à trier, alors que l'étalement et la dictée précisaient déjà d'office les cartes à édition unique. La ligne retient désormais, tant qu'on n'a pas statué : l'unique édition quand le catalogue n'en connaît qu'une (`sole_editions`, un aller-retour par langue pour toute la liste) ; sinon l'édition de cette carte dont on possède le plus d'exemplaires — `card_printings` les trie déjà ainsi, la plus récente départageant deux éditions à égalité, et une ligne suffit ; sinon rien, et la carte part à trier. C'est l'exception que le garde-fou §IV.8 fait pour cet écran : l'édition proposée s'affiche avant l'appui — sélecteur, illustration, prix —, on la confronte à la carte qu'on tient, et « + » vaut choix. « Ne pas préciser » l'écarte pour de bon, et une édition qui n'existe qu'en brillant est proposée brillante. Chaque ligne porte la clé de sa carte : réutilisée pour une autre, elle lui aurait transmis l'édition retenue pour la précédente.
...
### La dictée précise l'édition, sans la demander pendant l'écoute
...
**Ce que le catalogue tranche.** La dictée appelle `soleEditions` sur ce qui vient d'être entendu — groupé par langue, au plus deux requêtes par énoncé — et retient l'édition des cartes qui n'en admettent qu'une. Elle l'**annonce** sur la tuile, comme l'exige le garde-fou §IV.8 pour une édition posée sans geste. Une panne du catalogue renvoie à l'état d'avant : la carte part sans édition, jamais perdue.

**Ce que l'utilisateur tranche, et quand.** Le reste se choisit sur la même ligne qu'à l'étalement — `EditionLine`, partagée par les deux écrans depuis `printings/presentation/`, sélecteur d'édition et pastille de finition compris. L'argument « mains occupées » ne vaut que pendant l'écoute : une feuille modale ouverte là masquerait la liste au moment où les cartes s'y ajoutent. Il tombe dès que le micro est coupé, c'est-à-dire au moment où l'on relit sa liste avant de l'enregistrer. La ligne est donc **éteinte tant que le bouton affiche « Arrêter »**, active ensuite. C'est l'état du bouton qui la commande, non celui du moteur, qui s'interrompt seul entre deux phrases et la rallumerait sous le doigt en pleine dictée.

**Écarter une édition est un geste, pas un oubli.** Choisir « ne pas préciser » laisse la ligne nulle, exactement comme une carte jamais examinée ; sans marque, la phrase suivante relancerait le remplissage d'office et rendrait l'édition qu'on venait d'écarter. Un drapeau par ligne distingue les deux, et `soleEditions` ne réclame plus que ce qui n'a jamais été tranché.

**Un défaut trouvé en écrivant le test.** `dispose()` appelait `ref.read(speechServiceProvider).stop()`, ce que Riverpod refuse — « Using "ref" when a widget is about to or has been unmounted is unsafe », `ref` s'appuyant sur un `BuildContext` qui ne vaut plus rien. Quitter l'écran pouvait donc laisser l'écoute ouverte, et avec elle l'audio qui continue de sortir de l'appareil vers le moteur du système. Le service est désormais retenu dans un champ dès le montage, remède que Riverpod indique lui-même. L'écran de dictée n'avait aucun test ; il en a huit.
